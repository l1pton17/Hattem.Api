...
namespace Hattem.Api.Tests.Fluent.OnSuccess
{
    ...
    {
        [Fact(DisplayName = "(Async ValueTask, Async ValueTask) Should execute onSuccess if response is ok")]
        public async Task AsyncValueTask_AsyncValueTask_IsOk_ExecuteOnSuccess()
        {
        ...
        [Fact(DisplayName = "(Async ValueTask, Async ValueTask) Shouldn't execute onSuccess if response has errors")]
        public async Task AsyncValueTask_AsyncValueTask_HasErrors_DoesNotExecuteOnSuccess()
        {
            const int expectedData = 2;

...
namespace Hattem.Api.Tests.Fluent
{
    ...
    {
        [Fact(DisplayName = "(Async, Sync) Should execute onSuccess if response is ok")]
        public async Task Async_Sync_IsOk_ExecuteOnSuccess()
        {
        ...
        [Fact(DisplayName = "(Async, Sync) Shouldn't execute onSuccess if response has errors")]
        public async Task Async_Sync_HasErrors_DoesNotExecuteOnSuccess()
        {
            const int expectedData = 2;
